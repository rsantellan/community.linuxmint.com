...
class DX_Auth
{
	...
	*/
	...
	// Check if user is a Mint tester
	function is_tester()
	{		
		if ($this->ci->session->flashdata('istester_in_session')) {
				return $this->ci->session->flashdata('istester');
		} 
		else {
			if ($this->is_logged_in()) {
				...
				if ($users->num_rows() > 0) {
					$user = $users->row();
					$value = ($user->istester == 1);
					$this->ci->session->set_flashdata('istester_in_session', True);
					$this->ci->session->set_flashdata('istester', $value);
					return $value;
				}
				else {
					return False;
	...
	// Check if user is a Mint moderator
	function is_moderator()
	{
		if ($this->ci->session->flashdata('ismoderator_in_session')) {
				return $this->ci->session->flashdata('ismoderator');
		} 
		else {
			if ($this->is_logged_in()) {
				$this->ci->db->where('id', $this->get_user_id());
				...
				if ($users->num_rows() > 0) {
					$user = $users->row();
					$value = ($user->ismoderator == 1);
					$this->ci->session->set_flashdata('ismoderator_in_session', True);
					$this->ci->session->set_flashdata('ismoderator', $value);
					return $value;
				}
				else {
					return False;
	...
	// Check if user is a Mint maintainer
	function is_maintainer()
	{
		if ($this->ci->session->flashdata('ismaintainer_in_session')) {
				return $this->ci->session->flashdata('ismaintainer');
		} 
		else {
			if ($this->is_logged_in()) {
				$this->ci->db->where('id', $this->get_user_id());
				...
				if ($users->num_rows() > 0) {
					$user = $users->row();
					$value = ($user->ismaintainer == 1);
					$this->ci->session->set_flashdata('ismaintainer_in_session', True);
					$this->ci->session->set_flashdata('ismaintainer', $value);
					return $value;
				}
				else {
					return False;
	...
	// Check if user is a Mint admin
	function is_administrator()
	{
		return ($this->ci->session->userdata('DX_logged_in') && $this->ci->session->userdata('DX_role_id') == 2);
	}
